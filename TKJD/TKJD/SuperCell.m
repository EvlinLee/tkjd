...
-(id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        self.priceLab.textColor=UIColorFromRGB(0x555555);
        ...
        self.qPriceBtn=[UIButton buttonWithType:UIButtonTypeCustom];
        [self.qPriceBtn setTitleColor:UIColorFromRGB(0xff4e36) forState:UIControlStateNormal];
        self.qPriceBtn.titleLabel.font=[UIFont systemFontOfSize:12];
        [self.qPriceBtn setBackgroundImage:[UIImage imageNamed:@"super_bg"] forState:UIControlStateNormal];
        [self.contentView addSubview:self.qPriceBtn];
...
-(void)setModel:(ShopModel *)model{
    ...
    NSString * title =[NSString stringWithFormat:@"%d",[model.yhPrice intValue]];
        
    [self.qPriceBtn setTitle:title forState:UIControlStateNormal];
        
    CGSize size =[title sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12]}];
        
    NSString * qh =[NSString stringWithFormat:@"￥%.2f",[model.quanhou floatValue]];
    
    self.qhPriceLab.text=qh;

        
    NSString * priceStr =[NSString stringWithFormat:@"￥%.2f",[model.shoujia floatValue]];
    ...
    self.priceLab.attributedText=priceAttributedStr;

    
    
    if (IPHONE4||IPHONE5) {
        
        [self.qhPriceLab mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        [self.qhPriceLab setContentHuggingPriority:UILayoutPriorityRequired
                                           forAxis:UILayoutConstraintAxisHorizontal];
        [self.qhPriceLab setContentCompressionResistancePriority:UILayoutPriorityRequired
                                                         forAxis:UILayoutConstraintAxisHorizontal];
        ...
        [self.priceLab mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.equalTo(self.titleLab.mas_bottom).offset(0);
            make.left.equalTo(self.qhPriceLab.mas_right).offset(2);
            make.height.offset(20);
            
        }];
        
        [self.priceLab setContentHuggingPriority:UILayoutPriorityRequired
                                         forAxis:UILayoutConstraintAxisHorizontal];
        [self.priceLab setContentCompressionResistancePriority:UILayoutPriorityRequired
                                                       forAxis:UILayoutConstraintAxisHorizontal];
