...
#import "YBLabel.h"
@interface NetChainViewController ()<UIScrollViewDelegate,UITextViewDelegate>{
...
@implementation NetChainViewController
...
//    NSString * rate =[defaults objectForKey:@"rate"];
    ...
    [self.wrapperView sd_addSubviews:@[headerImg,titleLab,rateLab]];
    ...
    if ([self.model.yhPrice intValue]==0) {
        UILabel * qhLab=[[UILabel alloc]init];
        qhLab.font=[UIFont systemFontOfSize:14];
        qhLab.textColor=UIColorFromRGB(0x666666);
        
        NSString * qhStr =[NSString stringWithFormat:@"价格:￥%.2f",[self.model.shoujia floatValue]];
        ...
        [self.wrapperView sd_addSubviews:@[qhLab,salesLab]];
        ...
        [self.wrapperView sd_addSubviews:@[qhLab,salesLab,priceLab,yhPriceLab]];
