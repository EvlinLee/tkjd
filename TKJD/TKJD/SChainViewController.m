...
#import "ShareView.h"
#import "ZFPlayer.h"
@interface SChainViewController ()<UIScrollViewDelegate,ZFPlayerDelegate,UITextViewDelegate>{
...
@implementation SChainViewController
...
//    [self requestToken];
    ...
    NSLog(@">>>>%lld--%lld",[[defaults objectForKey:@"valid_time"] longLongValue],[[Customer timeString] longLongValue]);
...
- (void)setupImageCell{
    ...
    [headerImg addSubview:playBtn];
    [playBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.center.equalTo(headerImg);
        make.width.height.mas_equalTo(60);
    }];
    
    if ([self.model.video_url isEqualToString:@""]||self.model.video_url==NULL) {
    ...
    NSString * qhStr =[NSString stringWithFormat:@"券后￥%.2f",[self.model.quanhou floatValue]];
    ...
    NSString * priceStr =[NSString stringWithFormat:@"￥%.2f",[self.model.shoujia floatValue]];
    ...
    if ([self.model.me isEqualToString:@""]) {
        ...
        NSString * syStr =[NSString stringWithFormat:@"%d元优惠券,剩余数量%@/%@",[self.model.yhPrice intValue],self.model.yhqsy,num];
        NSRange syRang = [syStr rangeOfString:@","];
...
-(void)playClick{
    ...
    LRLog(@"%@",self.model.video);
    ZFPlayerModel *playerModel = [[ZFPlayerModel alloc] init];
    playerModel.videoURL         = [NSURL URLWithString:self.model.video];
    playerModel.scrollView       = self.scroollView;
    
    playerModel.fatherView = headerImg;
    
    playerModel.placeholderImageURLString=self.model.thumb;
...
-(void)requestActivity{
    if ([self.model.yhLink rangeOfString:@"activity_id"].location!=NSNotFound) {
        
        activityId=[[[[self.model.yhLink componentsSeparatedByString:@"activity_id="] lastObject]  componentsSeparatedByString:@"&"] firstObject];
        
        
    }else if ([self.model.yhLink rangeOfString:@"activityid"].location!=NSNotFound){
        
        activityId=[[[[self.model.yhLink componentsSeparatedByString:@"activityid="] lastObject] componentsSeparatedByString:@"&"] firstObject];
    ...
    [MHNetworkManager getRequstWithURL:@"https://uland.taobao.com/cp/coupon" params:@{@"itemId":self.model.goodsId,@"activityId":activityId} successBlock:^(id returnData, int code, NSString *msg) {
        
        if ([returnData[@"success"] intValue]==1) {
            
            if (returnData[@"result"][@"retStatus"]==0) {
...
-(void)requestApi{
    ...
    [operationManager POST:url parameters:@{@"goods_id":weakSelf.model.goodsId,@"me":weakSelf.model.me,@"pid":weakSelf.pid,@"token":[defaults objectForKey:@"access_token"]} success:^(AFHTTPRequestOperation *operation, id responseObject) {
        
        LRLog(@"%@",operation.responseObject);
        if ([operation.responseObject[@"status_code"] integerValue]==200) {
            
            NSLog(@">>>%@",weakSelf.model.me);
            NSString * url =operation.responseObject[@"resp"][@"result"][@"data"][@"coupon_click_url"];
            if (![weakSelf.model.me isEqualToString:@""]) {
                ...
                if (isOk==YES) {
                    ...
                    coupon_click_url=[NSString stringWithFormat:@"%@&activityId=%@",url,activityId];
